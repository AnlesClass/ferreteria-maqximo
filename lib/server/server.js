...
app.get('/imagen', (req, res) => {
    ...
    if (!imagePath) {
        return res.status(400).send('La ruta de la imagen es requerida.');
    }

    // Construir la ruta absoluta dentro de tu directorio "images"
...
app.get('/productos/get/names', async (req, res) => {
    ...
    sql = (name)
    ? 'SELECT p.nombre FROM Productos p WHERE p.nombre LIKE :name ORDER BY p.nombre FETCH FIRST 20 ROWS ONLY'
    : 'SELECT p.nombre FROM Productos p ORDER BY p.nombre FETCH FIRST 20 ROWS ONLY';
...
app.get('/existencias/get/all', async (req, res) =>{
    ...
    try {
        const { idSede, name } = req.query;

        const queryName = `${name}%`;
        
        const response = await conexion.execute(
            'SELECT p.idproducto,p.nombre,e.fecha_registro,e.precio,e.cantidad FROM Existencias e INNER JOIN Productos p ON e.idproducto = p.idproducto WHERE e.idsede = :idSede AND p.nombre LIKE :queryName ORDER BY p.nombre FETCH FIRST 15 ROWS ONLY',
            { idSede: idSede, queryName: queryName},
            { outFormat: oracledb.OUT_FORMAT_OBJECT}
        );

        if (response.rows == 0) {
            res.status(404).send("No se encontró el Producto o Sede.");
            return
        }
