...
app.get('/existencias/get/web/all', async (req, res) => {
    ...
    try {
        const response = await conexion.execute(
            'SELECT p.path_imagen,p.nombre,p.descripcion,e.precio,e.cantidad FROM Productos p INNER JOIN Existencias e ON p.idProducto=e.idProducto WHERE e.idsede=:idSede ORDER BY p.nombre',
            { idSede: idSede },
            { outFormat: oracledb.OUT_FORMAT_OBJECT }
        );
